...
impl<'ctx> CodeGen<'ctx> {
    ...
    fn generate_function(&mut self, name: &str, _params: &[String], body: &[Statement]) -> Result<(), String> {
        ...
        // Return 0 if not already returned
        let _ = self.builder.build_return(Some(&i32_type.const_int(0, false)));
    ...
    fn generate_statement(&mut self, stmt: &Statement) -> Result<(), String> {
        match stmt {
            ...
            _ => {
                // TODO: Implement other statements (Break, Continue, Return, etc.)
            }
        }
        Ok(())
    }
    ...
    fn generate_if(&mut self, condition: &Condition, then_block: &[Statement], 
        ...
        // Determine the else block (or first elif)
        ...
        if !elif_blocks.is_empty() || else_block.is_some() {
            ...
            if !elif_blocks.is_empty() {
                ...
                let next_elif_bb = if elif_blocks.len() > 1 {
                    self.context.append_basic_block(current_fn, "elif_next")
                } else if else_block.is_some() {
                    self.context.append_basic_block(current_fn, "else_block")
                } else {
                    end_bb
                ...
                self.builder.build_conditional_branch(elif_cond_val, elif_body_bb, next_elif_bb).map_err(|e| e.to_string())?;
                ...
                self.builder.build_unconditional_branch(end_bb).map_err(|e| e.to_string())?;
                self.builder.position_at_end(next_elif_bb);
                
                // TODO: Handle remaining elif blocks
            }
    ...
    fn generate_for(&mut self, var: &str, items: &[Expression], body: &[Statement]) -> Result<(), String> {
        ...
        if items.len() == 1 {
            if let Expression::Literal(crate::control_flow::Value::Number(n)) = items[0] {
                // Create loop counter
                let counter = self.builder.build_alloca(i64_type, var).map_err(|e| e.to_string())?;
                ...
                self.variable_types.insert(var.to_string(), "i64".to_string());
                
                self.builder.build_unconditional_branch(loop_block).map_err(|e| e.to_string())?;
                
                self.builder.position_at_end(loop_block);
                let counter_val = self.builder.build_load(i64_type, counter, "counter").map_err(|e| e.to_string())?;
                let limit = i64_type.const_int(n as u64, false);
    ...
    fn generate_expression(&mut self, expr: &Expression) -> Result<BasicValueEnum<'ctx>, String> {
        match expr {
            Expression::Literal(val) => {
                ...
                match val {
                    Value::Number(n) => Ok(self.context.i64_type().const_int(*n as u64, false).into()),
                    Value::String(s) => {
            ...
            if args.len() == 1 {
                ...
                match arg_val {
                    ...
                    BasicValueEnum::IntValue(int_val) => {
                        // Integer argument
                        let format_str = self.context.const_string(b"%ld\n", true);
                        let global_format = self.module.add_global(format_str.get_type(), None, "format");
                        global_format.set_initializer(&format_str);
                        let format_ptr = self.builder.build_pointer_cast(
                            global_format.as_pointer_value(),
